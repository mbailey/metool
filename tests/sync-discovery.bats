...
@test "_mt_find_repos_file should find .repos.txt in current directory" {
  echo "test-repo" > .repos.txt
  
  run _mt_find_repos_file
  [ "$status" -eq 0 ]
  [[ "$output" == *"/.repos.txt" ]]
}
...
@test "_mt_find_repos_file should prefer .repos.txt over repos.txt" {
...
@test "_mt_find_repos_file should return error when no file found" {
  run _mt_find_repos_file
  [ "$status" -eq 1 ]
  [ "$output" = "" ]
}

@test "_mt_find_repos_file should use specific filename when provided" {
  echo "custom-repo" > custom.txt
  
  run _mt_find_repos_file "$(pwd)" "custom.txt"
  [ "$status" -eq 0 ]
  [[ "$output" == *"/custom.txt" ]]
}

@test "_mt_find_repos_file should honor MT_SYNC_FILE environment variable" {
...
@test "_mt_find_repos_file should search up directory tree in git repo" {
...
@test "_mt_find_repos_file should stop at git repository root" {
...
@test "_mt_find_repos_file should find repos.txt in subdirectory of git repo" {
  # Create a git repository
  git init --quiet
  git config user.email "test@example.com"
  git config user.name "Test User"
...
@test "_mt_find_repos_file should only check current directory outside git repo" {
...
@test "_mt_find_repos_file should work with directory argument" {
  mkdir -p testdir
  echo "dir-repo" > testdir/.repos.txt
  
  run _mt_find_repos_file "testdir"
  [ "$status" -eq 0 ]
  [[ "$output" == *"/testdir/.repos.txt" ]]
}

@test "_mt_sync_parse_args should use discovery for no arguments" {
...
@test "_mt_sync_parse_args should find repos in directory argument" {
  mkdir -p external
  echo "vendor/ui-lib" > external/.repos.txt
  
  run _mt_sync_parse_args "external"
  [ "$status" -eq 0 ]
  echo "$output" | grep -q "REPOS_FILE=.*/external/.repos.txt"
  echo "$output" | grep -q "WORK_DIR=.*/external"
}

@test "_mt_sync_parse_args should error when no repos file found" {
...
@test "_mt_sync_parse_args should handle explicit file argument" {
  echo "explicit-repo" > my-repos.txt
  
  run _mt_sync_parse_args "my-repos.txt"
  [ "$status" -eq 0 ]
  echo "$output" | grep -q "REPOS_FILE=.*/my-repos.txt"
}

@test "_mt_sync_parse_args should error for missing explicit file" {
  run _mt_sync_parse_args "nonexistent.txt"
  [ "$status" -eq 1 ]
  echo "$output" | grep -q "ERROR: Path not found: nonexistent.txt"
}

@test "_mt_sync_parse_args should error for directory with no repos file" {
  mkdir -p empty-dir
  
  run _mt_sync_parse_args "empty-dir"
  [ "$status" -eq 1 ]
  echo "$output" | grep -q "ERROR: No repos.txt or .repos.txt found in directory: empty-dir"
}

@test "_mt_sync_parse_args should handle --file option" {
  echo "custom-repo" > custom-file.txt
  
  run _mt_sync_parse_args --file custom-file.txt
  [ "$status" -eq 0 ]
  echo "$output" | grep -q "REPOS_FILE=.*/custom-file.txt"
}

@test "_mt_sync_parse_args should error for missing --file" {
  run _mt_sync_parse_args --file nonexistent.txt
  [ "$status" -eq 1 ]
